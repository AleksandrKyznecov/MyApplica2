package com.example.myapplica2
...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        class CrsTransformation {
            ...
            fun main() {
                toastme(("Implicit/default EpsgNumber for latitude/longitude: ${inputCoordinate.crsIdentifier.epsgNumber} \n")) // 4326
                toastme("InputCoordinate: $inputCoordinate \n")
                toastme("TargetCrsIdentifier: $targetCrsIdentifier \n")
                toastme("Coordinate transformations from EPSG ${EpsgNumber.WORLD__WGS_84__4326} to EPSG ${EpsgNumber.SWEDEN__SWEREF99_TM__3006} \n")
                ...
                crsTransformationResults.forEach { result ->
                    ...
                    toastme("$implementationType: \nX / Y ===> ${outputCoordinate.getX()} / ${outputCoordinate.getY()} \n")
                }

                //toastme("Short names of all Leaf implementations:")
                val allLeafs = CrsTransformationAdapterLeafFactory.getInstancesOfAllKnownAvailableImplementations()
                allLeafs.forEach { leaf ->
                    //toastme(leaf.getShortNameOfImplementation())
                }
